  ...
  for (let r = 2; r < summeryRows.length; r++) {
    ...
    if (colScheme && colScheme !== "Scheme" && colScheme !== "Total" && colScheme !== "N/A") {
      ...
      schemes.push({
        ...
        septemberProducedM3: Number(row[8] || 0),
        totalProducedM3: Number(row[8] || 0) + Number(row[6] || 0) + Number(row[5] || 0),
        julySoldM3: Number(row[11] || 0),
        augustSoldM3: Number(row[12] || 0),
        septemberSoldM3: Number(row[14] || 0),
        totalSoldM3: Number(row[14] || 0) + Number(row[12] || 0) + Number(row[11] || 0),
