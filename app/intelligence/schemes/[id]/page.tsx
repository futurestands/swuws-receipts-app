...
export default function SchemeIntelligencePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const loadData = () => {
    startTransition(async () => {
      try {
        ...
        console.error("Failed to load scheme intelligence:", err)
      }
    })
  }
        ...
        <CardContent>
          {data.findings && data.findings.length > 0 ? (
            ...
            <div className="text-center py-6 space-y-1">
              ...
              <p className="text-xs font-bold text-foreground">No active intelligence findings for this scheme.</p>
            </div>
          )}
        </CardContent>
      </Card>
